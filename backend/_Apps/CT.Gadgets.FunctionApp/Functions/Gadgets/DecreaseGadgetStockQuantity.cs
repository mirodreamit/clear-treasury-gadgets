...
public class DecreaseGadgetStockQuantity(ILogger<DecreaseGadgetStockQuantity> log, IMediator mediator, IHttpRequestProcessingService httpRequestProcessingService)
{
    private readonly ILogger<DecreaseGadgetStockQuantity> _logger = log;
    ...
    private const string OperationId = $"{Tag}-decrease-stock";
    ...
    public async Task<IActionResult> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "patch", Route = $"{Version}/{Tag}/{{id}}/decrease-stock-by-one")] HttpRequest req,
        Guid id,
